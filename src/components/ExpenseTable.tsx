...
import { Trash, File, FileX } from 'lucide-react';
...
export const ExpenseTable = ({ refreshTrigger, fromDate, toDate }: ExpenseTableProps) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    fetchExpenses();
  }, [user, refreshTrigger, fromDate, toDate]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('unpaid-expenses-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>My Unpaid Expenses</CardTitle>
      </CardHeader>
      <CardContent>
        {expenses.length === 0 ? (
